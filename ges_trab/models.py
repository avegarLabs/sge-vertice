...
class Trabajador(BaseUrls):
    ...
    segundo_nombre = models.CharField(max_length=20, blank=True, null=False, default="''", validators=[person_name_validator])
    ...
    escala_salarial_ref = models.ForeignKey(adm.EscalaSalarialReforma, blank=True, null=True, on_delete=models.DO_NOTHING, verbose_name='escala salarial referencia')
    salario_escala_ref = models.DecimalField('salario escala referencia', max_digits=10, decimal_places=2, default=0.0)
    ...
    @property
    def grupo_escala_reforma(self):
        grupo_escala = adm.EscalaSalarialReforma.objects.get(grupo=self.escala_salarial.grupo).grupo
        if self.categoria is 'T' or self.categoria is 'C':
