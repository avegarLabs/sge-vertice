...
class Trabajador(BaseUrls):
    ...
    @property
    def grupo_escala_reforma_salario_Total(self):
        grupo_e = adm.EscalaSalarial.objects.get(grupo=self.escala_salarial).grupo
        if grupo_e == 'V':
            grupo_e = 'VIII'
            sal_total = adm.EscalaSalarialReforma.objects.get(grupo=grupo_e).salario_escala
            if self.cat_cient == '2':
                sal_total += 400  # valore referente no real todo verificar valores reales
            elif self.cat_cient == '3':
                sal_total += 800  # valore referente no real todo verificar valores reales
    ...
    @property
    def grupo_escala_reforma_salario_escala(self):
        grupo_e = adm.EscalaSalarial.objects.get(grupo=self.escala_salarial).grupo
        if grupo_e == 'V':
            grupo_e = 'VIII'
            return adm.EscalaSalarialReforma.objects.get(grupo=grupo_e).salario_escala
        if grupo_e == 'VI':
            grupo_e = 'IX'
            return adm.EscalaSalarialReforma.objects.get(grupo=grupo_e).salario_escala
        if grupo_e == 'VII':
    ...
    def grupo_escala_reforma(self):
        ...
        if grupo_e == 'V':
            grupo_e = 'VIII'
            return adm.EscalaSalarialReforma.objects.get(grupo=grupo_e).grupo
        if grupo_e == 'VI':
            grupo_e = 'IX'
            return adm.EscalaSalarialReforma.objects.get(grupo=grupo_e).grupo
        if grupo_e == 'VII':
